...
Baseline for this index: frontend **12.2.33**, canonical engine **1.2.4**, gm-copilot **v25**, source baseline **e8f81ea** plus the [passive event projection repair](passive-event-target-audit-2026-09-08.md). The latest completed JavaScript run contains **560 passing tests, zero failures and zero skips**, including the actual supplied Transformers DOCX. Lint/typecheck scripts are JavaScript syntax checks, not full static type analysis; the build script validates the static deployment, not a bundled application.
...
| 50 | Checks | Latest suite 560/560, actual DOCX, syntax checks, static build and diff checks pass. | Script called typecheck is syntax only; no false TypeScript guarantee. |
