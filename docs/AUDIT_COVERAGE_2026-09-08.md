...
Baseline for this index: frontend **12.2.34**, canonical engine **1.2.4**, gm-copilot **v25**, with the [passive event projection repair](passive-event-target-audit-2026-09-08.md) and [resolution accounting notice](resolution-accounting-display-audit-2026-09-08.md). The latest completed JavaScript run contains **565 passing tests, zero failures and zero skips**, including the actual supplied Transformers DOCX. Lint/typecheck scripts are JavaScript syntax checks, not full static type analysis; the build script validates the static deployment, not a bundled application.
...
| 15 | Passives | Known triggers execute automatically. Owned/borrowed role identity and exact official passive-event target arrays are verified. [Sources](passive-source-audit-2026-09-08.md), [event targets](passive-event-target-audit-2026-09-08.md). | General custom event dispatch, all listed events and passive reward causality still open. |
...
| 50 | Checks | Latest suite 565/565, actual DOCX, syntax checks, static build and diff checks pass. | Script called typecheck is syntax only; no false TypeScript guarantee. |
