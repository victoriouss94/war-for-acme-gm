...
## Follow-up: owner-scoped mechanic review identity (12.2.23)

The next pass reproduced a broader collision: two duplicated roles produced two local review rows with one shared source ID, and the UI's ID-only map hid one. The database review function likewise reduced four role/ability owners with a shared mechanic ID to one result per game. Cross-game UI merging also collapsed matching IDs from separate games. Five of six initial application regressions failed.

The local review queue and existing UI merge now use an unambiguous tuple of game, role, ability and original review ID. Local entries still supersede matching remote entries, but cannot replace another owner's review. The original id fields and source mechanics are not rewritten. The existing get_mechanics_review_queue function uses the same owner scope when deduplicating. Production migration 20260908161205 changes only this SELECT identity; SECURITY INVOKER, empty search path, caller authorization and existing grants are retained. No data rows were migrated.

Six application tests cover role/game/ability separation, standalone versus role-owned mechanics, canonical/snake-case remote fields, local precedence and delimiter safety. Authenticated rollback fixtures reproduced one result per game before the repair and verified four per game afterward, eight across two authorized games. Unrelated and missing identities returned no fixture reviews; anon EXECUTE remains denied. All four before/after synthetic games were verified absent. Feeding the actual returned database JSON into the real UI merge retained all eight entries and correctly overlaid a local update.

All 476 tests pass, zero skipped, with the supplied Transformers DOCX; syntax/static build/diff checks pass. Security advisors retain the same 46 underlying findings (45 WARN, 1 INFO). Live Transformers remains Night 1/version 192. No live game, uploaded file, account or provider usage was changed; no Edge deployment was required.

This repairs display visibility even for already-copied records sharing source mechanic IDs. It does not resolve their mechanics or infer missing definitions. Different mechanics incorrectly sharing an ID within the same owner, id-less derived-review stability, other import/review parity issues and the broader audit remain outside this fix.
