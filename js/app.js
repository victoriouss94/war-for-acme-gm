...
async function openGame(gameId,viewId='dashboardView'){
  ...
  cloudAudit=[];cloudImports=[];cloudInvites=[];phaseOne={documents:[],abilities:[],modifiers:[],conversationId:null,messages:[]};$('inviteResult').hidden=true;$('inviteFormPanel').hidden=true;gameIndex.activeGameId=gameId;state=loadGameData(gameId);resetEditorContext();saveIndex();renderAll();showView(viewId);
  await refreshOpenGame();await subscribeToOpenGame();
}
function unloadCurrentGame(){
  GMCloud.unsubscribe();cloudChannelGameId=null;cloudAudit=[];cloudImports=[];cloudInvites=[];phaseOne={documents:[],abilities:[],modifiers:[],conversationId:null,messages:[]};gameIndex.activeGameId=null;state=null;resetEditorContext();saveIndex();renderAll();showView('gamesView');
}
...
function beginRoleEdit(roleId){
  ...
  selectedRoleAbilityIds=new Set(state.abilities.filter(ability=>role.tags.some(tag=>normalized(tag)===normalized(ability.name))).map(ability=>ability.id));renderRoleAbilityPicker();renderRoleModifierEditor();$('roleName').focus();
}
...
function copilotSources(sources=[]){return sources.length?'<details class="copilot-sources"><summary>Sources ('+sources.length+')</summary>'+sources.map(source=>'<article><strong>'+esc(source.title)+'</strong><span>v'+esc(source.version)+' • '+esc(source.locator||source.kind)+'</span><p>'+esc(source.claim||source.excerpt)+'</p>'+(source.excerpt&&source.claim?'<blockquote>'+esc(source.excerpt)+'</blockquote>':'')+'</article>').join('')+'</details>':''}
...
async function askCopilot(messageOverride='',taskOverride=''){
  ...
  let request;
  try{request=normalizeCopilotRequest({message,task,depth,conversationId:phaseOne.conversationId,history:thread.map(entry=>({role:entry.role,content:entry.content||entry.result?.answer||''}))})}catch(error){return setCopilotStatus(error.message,'error')}
  ...
  try{
    ...
    phaseOne.conversationId=response.conversationId||phaseOne.conversationId;thread.push({role:'assistant',content:result.answer,result,model:response.model,gameVersion:response.gameVersion,generatedAt:response.generatedAt,applied:false});setCopilotStatus(result.requires_gm_decision?'Answer ready. The cited sources leave a decision for the GM.':'Answer ready. Review any proposed changes before applying them.','success');
...
function renderKnowledgeLibrary(){
  ...
  $('uploadKnowledgeBtn').disabled=knowledgePending||!canEditGame()||!selectedKnowledgeFile;$('importKnowledgeAsSetupBtn').disabled=knowledgePending||!canEditGame();$('knowledgeFile').disabled=knowledgePending||!canEditGame();$('knowledgeTitle').disabled=knowledgePending||!canEditGame();$('knowledgeType').disabled=knowledgePending||!canEditGame();$('knowledgeStatus').disabled=knowledgePending||!canEditGame();
  $('knowledgeDocumentList').innerHTML=documents.map(document=>{const versions=(document.official_document_versions||[]).slice().sort((a,b)=>b.version_number-a.version_number),latest=versions[0];return '<article class="knowledge-document"><div><strong>'+esc(document.title)+'</strong><p class="muted">'+esc(document.document_type.replaceAll('_',' '))+' • '+(latest?'v'+latest.version_number+' • '+latest.status:'No version')+'</p>'+(latest?.summary?'<p>'+esc(latest.summary)+'</p>':'')+(latest?.ingestion_error?'<p class="form-error">'+esc(latest.ingestion_error)+'</p>':'')+'</div></article>'}).join('')||'<p class="muted">No game-specific official documents have been uploaded.</p>';
}
async function uploadSelectedKnowledge(){
  ...
  try{const digest=await crypto.subtle.digest('SHA-256',await selectedKnowledgeFile.arrayBuffer()),sha256=[...new Uint8Array(digest)].map(value=>value.toString(16).padStart(2,'0')).join(''),documentId=id();await GMCloud.uploadKnowledgeDocument(currentGame().id,selectedKnowledgeFile,{title,documentType:$('knowledgeType').value,status:$('knowledgeStatus').value,documentKey:knowledgeDocumentKey(title,documentId),sha256,depth:$('copilotDepth').value});selectedKnowledgeFile=null;$('knowledgeFile').value='';$('knowledgeTitle').value='';await refreshPhaseOne(currentGame().id);setNotice($('knowledgeStatusMessage'),'Document indexed. The AI can now retrieve cited passages from active versions.','success')}
...
function renderOfficialAbilities(){
  ...
  $('officialAbilityList').innerHTML=rows.map(ability=>'<article class="ability-entry open official-ability-entry"><div class="ability-summary"><div><div class="ability-name">'+ability.sort_order+'. '+esc(ability.display_name)+'</div><div class="ability-definition">'+esc(ability.official_description||'Official definition text has not been supplied. No behavior was invented.')+'</div></div><span class="category-badge">'+esc(ability.category)+'</span></div><div class="ability-details"><p><code>'+esc(ability.ability_id)+'</code> <span class="badge">v'+ability.version_number+'</span> <span class="badge">'+esc(ability.version_scope)+'</span> <span class="badge">'+esc(ability.definition_status)+'</span></p><p class="muted">Source: '+esc(ability.source_title||ability.dataset_name)+' • version '+esc(ability.source_version||ability.version_number)+'</p><div class="ability-actions">'+(active&&canEditGame()?'<button class="secondary edit-official-ability" data-id="'+esc(ability.ability_id)+'">Create New Version</button>':'')+'</div></div></article>').join('')||'<div class="empty-state">No official abilities match this filter.</div>';
...
function renderAll(){
  ...
  renderSelects();renderRoleAbilityPicker();renderRoleEditorAccess();renderDashboard();renderFactions();renderRoles();renderPlayers();renderQueue();renderCopilot();renderStats();renderRules();renderEncyclopedia();renderOfficialAbilities();renderHistory();renderSettings();
}
...
$('activateOfficialDatasetBtn').onclick=async()=>{const datasetId=phaseOne.abilities[0]?.dataset_id;if(!datasetId||!canEditGame()||!confirm('Activate the Courtroom standardized ability dataset for this game? This makes it available to the AI and editors, but does not copy abilities or change any role.'))return;try{await GMCloud.activateAbilityDataset(currentGame().id,datasetId);await refreshPhaseOne(currentGame().id);renderAll()}catch(error){alert(error.message)}};
