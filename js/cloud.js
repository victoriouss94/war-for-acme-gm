(function(){
  ...
  async function playerModeState(gameId){const [states,events]=await Promise.all([required().from('player_mode_states').select('*').eq('game_id',gameId),required().from('player_mode_events').select('*').eq('game_id',gameId).order('created_at',{ascending:false}).limit(500)]);for(const result of [states,events])if(result.error)throw result.error;return {states:states.data||[],events:events.data||[]}}
  async function changePlayerMode(gameId,playerId,modeId,reason,sourceType='GM_TRIGGERED'){return unwrap(await required().rpc('change_player_mode',{target_game_id:gameId,target_player_id:playerId,target_mode_id:modeId,target_reason:reason,target_source_type:sourceType}))}
  async function mutateTemporaryModeAccess(gameId,playerId,modeId,operation,reason,expiresAt=null,expiresCycle=null,expiresPhase=''){return unwrap(await required().rpc('mutate_temporary_mode_access',{target_game_id:gameId,target_player_id:playerId,target_mode_id:modeId,target_operation:operation,target_reason:reason,target_expires_at:expiresAt,target_expires_cycle:expiresCycle,target_expires_phase:expiresPhase}))}
  async function playerAbilityState(gameId){
    const [grants,events,resolutionEvents]=await Promise.all([
      required().from('player_ability_grants').select('*').eq('game_id',gameId).order('created_at',{ascending:true}),
      required().from('player_ability_grant_events').select('*').eq('game_id',gameId).order('created_at',{ascending:false}).limit(500),
      required().from('resolution_session_events').select('*,resolution_sessions!resolution_session_events_session_id_fkey(cycle,phase)').eq('game_id',gameId).order('created_at',{ascending:false}).limit(500)
    ]);
    for(const result of [grants,events,resolutionEvents])if(result.error)throw result.error;
    return {grants:grants.data||[],events:events.data||[],resolutionEvents:(resolutionEvents.data||[]).map(event=>({...event,cycle:event.resolution_sessions?.cycle,phase:event.resolution_sessions?.phase,summary:event.outcome?.summary||''}))};
  }
  async function grantPlayerAbility(gameId,gameVersion,payload){return unwrap(await required().rpc('grant_player_ability',{target_game_id:gameId,expected_game_version:gameVersion,target_player_id:payload.playerId,target_ability_id:payload.abilityId,target_source_type:payload.sourceType||'GM_GRANT',target_source_reference:payload.sourceReference||'',target_reason:payload.reason||'',target_uses:payload.uses??null,target_duration_type:payload.durationType||'UNTIL_REMOVED',target_expires_at:payload.expiresAt||null,target_expires_cycle:payload.expiresCycle??null,target_expires_phase:payload.expiresPhase||'',target_phase_restrictions:payload.phaseRestrictions||[],target_special_conditions:payload.specialConditions||{},target_survives_conversion:payload.survivesConversion??null,target_stealable:payload.stealable!==false,target_metadata:payload.metadata||{}}))}
  async function bulkGrantPlayerAbilities(gameId,gameVersion,payloads){return unwrap(await required().rpc('bulk_grant_player_abilities',{target_game_id:gameId,expected_game_version:gameVersion,target_grants:payloads||[]}))}
  async function grantRandomPlayerAbility(gameId,gameVersion,playerId,abilityIds,payload){return unwrap(await required().rpc('grant_random_player_ability',{target_game_id:gameId,expected_game_version:gameVersion,target_player_id:playerId,target_ability_pool:abilityIds||[],target_source_reference:payload.sourceReference||'',target_reason:payload.reason||'',target_uses:payload.uses??1,target_duration_type:payload.durationType||'UNTIL_USED'}))}
  async function mutatePlayerAbilityGrant(grantId,version,operation,reason,usesRemaining=null){return unwrap(await required().rpc('mutate_player_ability_grant',{target_grant_id:grantId,expected_version:version,target_operation:operation,target_reason:reason||'',target_uses_remaining:usesRemaining}))}
  async function queuePlayerAction(gameId,gameVersion,payload,replaceActionId=null){return unwrap(await required().rpc('queue_player_action',{target_game_id:gameId,expected_game_version:gameVersion,target_action:payload||{},target_replace_action_id:replaceActionId}))}
  async function removeQueuedAction(gameId,gameVersion,actionId,reason,phaseId,phaseVersion){return unwrap(await required().rpc('remove_queued_action',{target_game_id:gameId,expected_game_version:gameVersion,target_action_id:actionId,target_reason:reason||'',target_phase_id:phaseId,target_phase_version:phaseVersion}))}
  async function phaseContext(gameId){return unwrap(await required().rpc('get_game_phase_context',{target_game_id:gameId}))}
  async function previewPhaseAdvance(gameId,phaseId,phaseVersion){return unwrap(await required().rpc('preview_game_phase_advance',{target_game_id:gameId,target_phase_id:phaseId,target_phase_version:phaseVersion}))}
  async function startGamePhase(gameId,gameVersion,startingPhase,reason){return unwrap(await required().rpc('start_game_phase',{target_game_id:gameId,expected_game_version:gameVersion,target_starting_phase:startingPhase,target_reason:reason||''}))}
  async function setGamePhasePause(gameId,gameVersion,phaseId,phaseVersion,paused,reason){return unwrap(await required().rpc('set_game_phase_pause',{target_game_id:gameId,expected_game_version:gameVersion,target_phase_id:phaseId,target_phase_version:phaseVersion,target_paused:Boolean(paused),target_reason:reason||''}))}
  async function advanceGamePhase(gameId,gameVersion,phaseId,phaseVersion,allowUnresolved,reason){return unwrap(await required().rpc('advance_game_phase',{target_game_id:gameId,expected_game_version:gameVersion,target_phase_id:phaseId,target_phase_version:phaseVersion,target_allow_unresolved:Boolean(allowUnresolved),target_reason:reason||''}))}
  async function adminCorrectPhaseAction(gameId,phaseId,phaseVersion,operation,actionId,action,reason){return unwrap(await required().rpc('admin_correct_phase_action',{target_game_id:gameId,target_phase_id:phaseId,expected_phase_version:phaseVersion,target_operation:operation,target_action_id:actionId,target_action:action||{},target_reason:reason||''}))}
  ...
  async function adjudicateInteraction(gameId,resolutionSessionId,interaction,depth='standard'){
    const {data,error}=await required().functions.invoke('gm-copilot',{body:{gameId,resolutionSessionId,task:'adjudicate_interaction',interaction,depth}});
    if(error){let payload=null;try{payload=await error.context?.json?.()}catch{}throw Object.assign(new Error(payload?.error||error.message||'The isolated AI adjudication failed.'),{code:payload?.code||'AI_ADJUDICATION_FAILED'})}
    return data?.adjudication||null;
  }
  async function phaseOneContext(gameId){
    ...
    const messages=conversationId?unwrap(await required().from('ai_messages').select('id,role,content,structured_result,model,game_version,intent,referenced_entities,tool_trace,created_at').eq('conversation_id',conversationId).order('created_at',{ascending:true}).limit(250)):[];
  ...
  async function resolutionContext(gameId){
    const [sessions,precedents,drafts,interactions,summary,usage,limit,patterns,concepts,mappings,proposals,runs,toolCalls,globalRules,effectiveRuleset,assignmentPreviews,assignmentHistory,mechanicsReviews,abilityUsage,usageAnalytics,usageMonth]=await Promise.all([
      ...
      required().rpc('get_resolution_usage_analytics',{target_game_id:gameId,target_filters:{}}),
      Promise.resolve(required().rpc('get_ai_usage_month_summary',{target_game_id:gameId})).catch(()=>({error:true}))
    ]);
    ...
    return {sessions:sessions.data||[],precedents:precedents.data||[],drafts:drafts.data||[],interactions:interactions.data||[],summary:summary.data||{},usage:usage.error?[]:usage.data||[],usageMonth:usageMonth.error?null:usageMonth.data||null,limit:limit.error?null:limit.data||null,patterns:patterns.data||{},concepts:concepts.data||[],mappings:mappings.data||[],proposals:proposals.data||[],runs:runs.data||[],toolCalls:toolCalls.data||[],globalRules:globalRules.data||[],mechanicsReviews:mechanicsReviews.error?[]:mechanicsReviews.data||[],abilityUsage:abilityUsage.error?[]:abilityUsage.data||[],usageAnalytics:usageAnalytics.error?{}:usageAnalytics.data||{},effectiveRuleset:effectiveRuleset.data||{gameRules:[],gameOverrides:[],globalFallbacks:[],standardAbilities:[],roleModifiers:[],unresolved:[]},assignmentPreviews:assignmentPreviews.data||[],assignmentHistory:assignmentHistory.data||[]};
  }
  ...
  async function saveDeterministicResolution(sessionId,lockVersion,proposal){return unwrap(await required().rpc('save_deterministic_resolution',{target_session_id:sessionId,expected_lock_version:lockVersion,target_engine_proposal:proposal||{}}))}
  async function finalizeResolutionSession(sessionId,lockVersion,decision,manualResolution,explanation,teachAi,teachScope='GLOBAL',consumedActionIds=[],idempotencyKey=crypto.randomUUID(),overrideWarnings=false){return unwrap(await required().rpc('approve_and_apply_resolution',{target_session_id:sessionId,expected_lock_version:lockVersion,target_final_resolution:manualResolution||{},target_gm_explanation:explanation||'',target_teach_ai:Boolean(teachAi),target_teach_scope:teachScope,target_consumed_action_ids:consumedActionIds||[],target_idempotency_key:idempotencyKey,target_override_warnings:Boolean(overrideWarnings),target_reject:decision==='REJECT'}))}
  ...
  async function approveAndAddAiDraft(draftId,version,entity){const rows=unwrap(await required().rpc('approve_and_add_ai_draft',{target_draft_id:draftId,expected_game_version:version,target_entity:entity}));return rows[0]}
  async function reviewAiChangeProposal(proposalId,version,decision,editedChanges=null,reason=''){return unwrap(await required().rpc('review_ai_change_proposal',{target_proposal_id:proposalId,expected_version:version,target_decision:decision,target_edited_changes:editedChanges,target_reason:reason||''}))}
  ...
  async function subscribe(gameId,{onDocument,onMembership,onInvites,onStatuses,onAbilities,onResolution,onPhase,onLearning,onPresence,onStatus}){
    await unsubscribe();
    ...
    channel.on('postgres_changes',{event:'*',schema:'public',table:'player_ability_grants',filter:'game_id=eq.'+gameId},payload=>onAbilities?.(payload));
    channel.on('postgres_changes',{event:'*',schema:'public',table:'player_ability_grant_events',filter:'game_id=eq.'+gameId},payload=>onAbilities?.(payload));
    ...
    channel.on('postgres_changes',{event:'*',schema:'public',table:'game_phases',filter:'game_id=eq.'+gameId},payload=>onPhase?.(payload));
    channel.on('postgres_changes',{event:'*',schema:'public',table:'player_mode_states',filter:'game_id=eq.'+gameId},payload=>onPhase?.(payload));
    channel.on('postgres_changes',{event:'*',schema:'public',table:'player_mode_events',filter:'game_id=eq.'+gameId},payload=>onPhase?.(payload));
    channel.on('postgres_changes',{event:'*',schema:'public',table:'game_phase_events',filter:'game_id=eq.'+gameId},payload=>onPhase?.(payload));
  ...
  window.GMCloud={init,passwordSignIn,createAccount,upgradeLegacyAccount,changePassword,signOut,listGames,loadGame,createGame,createImportedGame,reimportGame,saveGame,deleteGame,joinGame,invites,generateInvite,revokeInvite,setMemberRole,removeMember,roleTemplates,abilityTemplates,history,statusEffects,playerStatusHistory,playerState,mutatePlayerStatus,applyPlayerStatusChanges,playerModeState,changePlayerMode,mutateTemporaryModeAccess,playerAbilityState,grantPlayerAbility,bulkGrantPlayerAbilities,grantRandomPlayerAbility,mutatePlayerAbilityGrant,queuePlayerAction,removeQueuedAction,phaseContext,previewPhaseAdvance,startGamePhase,setGamePhasePause,advanceGamePhase,adminCorrectPhaseAction,imports,downloadImport,askCopilot,analyzeWordDocument,phaseOneContext,uploadKnowledgeDocument,activateAbilityDataset,createStandardAbilityVersion,standardAbilityHistory,saveRoleAbilityModifier,startNewAiConversation,resolutionContext,createRoleAssignmentPreview,shuffleRoleAssignmentPreview,cancelRoleAssignmentPreview,applyRoleAssignmentPreview,saveGlobalRule,resolutionEvents,startResolutionSession,saveDeterministicResolution,finalizeResolutionSession,reviewAiDraft,reviewAiChangeProposal,managePrecedent,promoteGlobalPattern,createGlobalAbilityConcept,approveAbilityConceptMapping,removeAbilityConceptMapping,setAiUsageLimit,subscribe,unsubscribe,track,user,account,dispose,normalizeUsername};
  window.GMCloud.adjudicateInteraction=adjudicateInteraction;
  window.GMCloud.approveAndAddAiDraft=approveAndAddAiDraft;
})();
