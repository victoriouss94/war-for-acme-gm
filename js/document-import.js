...
export function normalizeAiDocumentImport(result,source={}){
  ...
  for(const raw of (Array.isArray(result.roles)?result.roles:[]).slice(0,2000)){
    ...
    const explicitType=limitedText(raw.role_type,40).toUpperCase(),role=createRole(name,{factionTempId:faction?.tempId||null,factionName:faction?.name||factionName,alignment:limitedText(raw.alignment,500),description:limitedText(raw.description),activeAbilityName:limitedText(raw.active_ability_name,120),passiveAbilityName:limitedText(raw.passive_ability_name,120),abilityNames,abilityUses:raw.ability_uses==null?null:Math.max(0,Math.min(10000,Number(raw.ability_uses)||0)),cooldowns:limitedText(raw.cooldowns,1000),restrictions:limitedList(raw.restrictions,100,500),immunities:limitedList(raw.immunities,100,500),abilityModifiers,statusInteractions,relationships,unresolvedComponents:limitedList(raw.unresolved_components,100,500),winCondition:limitedText(raw.win_condition),notes:limitedText(raw.notes),gmNotes,tags:limitedList(raw.tags,100,120),roleType:explicitType===ROLE_TYPES.BASIC?ROLE_TYPES.BASIC:ROLE_TYPES.STANDARD,abilityDataStatus:explicitType===ROLE_TYPES.BASIC?ABILITY_DATA_STATUSES.INTENTIONALLY_NONE:abilityNames.length?ABILITY_DATA_STATUSES.COMPLETE:ABILITY_DATA_STATUSES.POSSIBLY_INCOMPLETE,basicEvidence:limitedText(raw.basic_evidence,1000),slotCount:Math.max(1,Math.min(1000,Number(raw.slot_count)||1)),enabled:raw.enabled!==false,confidence:confidence(raw.confidence,.82),sourceText:limitedText(raw.source_text||name,12000),sourceLocation:limitedText(raw.source_location,500)});
