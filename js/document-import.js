...
function createRole(name,fields={}){const clean=text(name);return {tempId:tempId('role'),name:clean,factionTempId:null,factionName:'',alignment:'',description:'',activeAbilityName:'',passiveAbilityName:'',abilityNames:[],abilityUses:null,cooldowns:'',restrictions:[],immunities:[],abilityModifiers:[],statusInteractions:[],relationships:[],winCondition:'',notes:'',gmNotes:'',tags:[],roleType:ROLE_TYPES.STANDARD,abilityDataStatus:ABILITY_DATA_STATUSES.POSSIBLY_INCOMPLETE,basicEvidence:'',slotCount:1,enabled:true,selected:true,confidence:.75,sourceText:clean,sourceLocation:'',duplicateOfTempId:null,duplicateDecision:'merge',...fields}}
...
export function prepareDocumentBlocksForAi(blocks){
  const prepared=(Array.isArray(blocks)?blocks:[]).map(block=>{
    ...
    return {type,level:type==='heading'?Math.min(6,Math.max(1,Number(block.level)||1)):null,text:text(block.text).slice(0,12000),boldLabel:text(block.boldLabel).slice(0,500),ordered:Boolean(block.ordered)};
...
export function normalizeAiDocumentImport(result,source={}){
  ...
  const ensureAbility=raw=>{
    ...
    if(!ability){ability=createAbility(name,{definition:limitedText(raw?.definition),category:['Investigation','Harmful','Protection','Support','Control','Communication','Passive','Other'].includes(raw?.category)?raw.category:inferAbilityCategory(name,raw?.definition,raw?.category),phase:['Night','Day','Any','Passive'].includes(raw?.phase)?raw.phase:inferAbilityPhase(raw?.category,raw?.definition),mechanics:limitedList(raw?.mechanics,50,120),mapping:raw?.mapping==='STANDARDIZED'?'STANDARDIZED':'CUSTOM',stableAbilityId:limitedText(raw?.standard_ability_id,120),modifiers:limitedList(raw?.modifiers,100,1000),interactions:limitedList(raw?.interactions,100,1000),confidence:confidence(raw?.confidence,.82),sourceText:limitedText(raw?.source_text||raw?.definition||name,2000),sourceLocation:limitedText(raw?.source_location,500)});model.abilities.push(ability);abilityMap.set(key,ability)}
  ...
  for(const raw of (Array.isArray(result.roles)?result.roles:[]).slice(0,2000)){
    ...
    const abilityNames=limitedList([...(Array.isArray(raw.ability_names)?raw.ability_names:[]),raw.active_ability_name,raw.passive_ability_name],100,120);abilityNames.forEach(abilityName=>ensureAbility({name:abilityName,definition:'',category:'Other',phase:'Any',confidence:.55,source_text:abilityName}));
    ...
    const explicitType=limitedText(raw.role_type,40).toUpperCase(),role=createRole(name,{factionTempId:faction?.tempId||null,factionName:faction?.name||factionName,alignment:limitedText(raw.alignment,500),description:limitedText(raw.description),activeAbilityName:limitedText(raw.active_ability_name,120),passiveAbilityName:limitedText(raw.passive_ability_name,120),abilityNames,abilityUses:raw.ability_uses==null?null:Math.max(0,Math.min(10000,Number(raw.ability_uses)||0)),cooldowns:limitedText(raw.cooldowns,1000),restrictions:limitedList(raw.restrictions,100,500),immunities:limitedList(raw.immunities,100,500),abilityModifiers,statusInteractions,relationships,winCondition:limitedText(raw.win_condition),notes:limitedText(raw.notes),gmNotes,tags:limitedList(raw.tags,100,120),roleType:explicitType===ROLE_TYPES.BASIC?ROLE_TYPES.BASIC:ROLE_TYPES.STANDARD,abilityDataStatus:explicitType===ROLE_TYPES.BASIC?ABILITY_DATA_STATUSES.INTENTIONALLY_NONE:abilityNames.length?ABILITY_DATA_STATUSES.COMPLETE:ABILITY_DATA_STATUSES.POSSIBLY_INCOMPLETE,basicEvidence:limitedText(raw.basic_evidence,1000),slotCount:Math.max(1,Math.min(1000,Number(raw.slot_count)||1)),enabled:raw.enabled!==false,confidence:confidence(raw.confidence,.82),sourceText:limitedText(raw.source_text||name,2000),sourceLocation:limitedText(raw.source_location,500)});
...
export function validateGameImport(model){
  ...
  const factionIds=new Set(factions.map(item=>item.tempId)),roleNames=new Set();roles.forEach(role=>{classifyImportedRole(role);const key=normalizeImportName(role.name);if(!key)errors.push('Every selected role needs a name.');else if(roleNames.has(key))errors.push('Selected role names must be unique: '+role.name+'.');else roleNames.add(key);if(!role.factionTempId||!factionIds.has(role.factionTempId))errors.push(role.name+' must be assigned to a selected faction.');if(role.roleType===ROLE_TYPES.BASIC&&role.abilityNames?.length)errors.push(role.name+' is Basic and cannot own role abilities.');if(role.abilityDataStatus===ABILITY_DATA_STATUSES.POSSIBLY_INCOMPLETE)warnings.push(role.name+' may be missing ability data; it was not silently classified as Basic.');});
...
import {ABILITY_DATA_STATUSES,ROLE_TYPES,classifyRoleAbilityData} from './player-setup.js?v=11.1.0';
