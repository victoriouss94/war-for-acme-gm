export const MECHANIC_SCHEMA_VERSION=1;
...
export function normalizeAbilityUnderstanding(ability={}){
  ...
  return {schemaVersion:MECHANIC_SCHEMA_VERSION,mechanics,targeting,baseStandardAbilityId,baseStandardAbilityName:text(raw.baseStandardAbilityName??raw.base_standard_ability_name,200),customIdentity,factionAction:Boolean(raw.factionAction??raw.faction_action??mechanics.some(item=>item.factionAction||['FACTION_BLOCK','FACTION_EFFECT'].includes(item.type))),globalAction:Boolean(raw.globalAction??raw.global_action??mechanics.some(item=>item.type==='GLOBAL_EFFECT'||item.targeting.type==='GLOBAL')),performerRequired:Boolean(raw.performerRequired??raw.performer_required),sourceFactionIds:list(raw.sourceFactionIds??raw.source_faction_ids,100,160),unresolvedComponents:list(raw.unresolvedComponents??raw.unresolved_components,100,500),sourceVersion:text(raw.sourceVersion??raw.source_version??ability.sourceVersion,160)};
}
