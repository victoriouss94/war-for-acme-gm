...
export const TARGET_TYPES=Object.freeze(['ONE_PLAYER','MULTIPLE_PLAYERS','SELF','NO_TARGET','DEAD_PLAYER','ABILITY','FACTION','CUSTOM_TARGET','OTHER']);
...
export function abilityTargeting(ability){
  const configured=record(ability?.targeting),requested=clean(configured.type??ability?.targetType,40).toUpperCase();
  return {type:TARGET_TYPES.includes(requested)?requested:'ONE_PLAYER',legacy:!requested,minTargets:Math.max(0,Number(configured.minTargets)||0),maxTargets:Math.max(1,Number(configured.maxTargets)||1),livingOnly:Boolean(configured.livingOnly),deadOnly:Boolean(configured.deadOnly),selfAllowed:configured.selfAllowed!==false,selfProhibited:Boolean(configured.selfProhibited),factionMemberOnly:Boolean(configured.factionMemberOnly),nonFactionMemberOnly:Boolean(configured.nonFactionMemberOnly),manuallyTriggerable:Boolean(configured.manuallyTriggerable??ability?.manuallyTriggerable)};
}
...
export function effectivePlayerAbilities({player,role,abilities=[],grants=[],statuses=[],resolutionEvents=[],roleModifiers=[],game={}}={}){
  ...
  for(const abilityId of abilityIdsForRole(role,abilities)){
    ...
    result.push({key:'ROLE:'+ability.id,abilityId:ability.id,baseAbilityId:ability.id,effectiveAbilityId:amplified?.id||ability.id,name:ability.name,effectiveName:amplified?.name||ability.name,definition:ability.definition,category:ability.category||'Custom',phase:ability.phase||'Any',mechanics:ability.mechanics||[],targeting:abilityTargeting(amplified||ability),sourceType:'ROLE',sourceLabel:'Role',sourceReference:role?.id||'',grantId:null,grantVersion:null,passive,usesGranted:baseUses,usesRemaining,additionalUses:additional,cooldownUntil,modifiers:roleModifiers.filter(modifier=>String(modifier.role_id??modifier.roleId)===String(role?.id)&&String(modifier.ability_id??modifier.abilityId)===ability.id),...availabilityState});
  }
  for(const rawGrant of grants){const grant=normalizeAbilityGrant(rawGrant);if(grant.playerId!==player.id||!grantIsCurrent(grant,game))continue;const ability=byId.get(grant.abilityId);if(!ability)continue;const passive=clean(ability.phase,30)==='Passive',amplified=amplifiedAbility(statuses,player.id,ability.id,abilities),cooldownCycles=Math.max(0,Number(ability.cooldownCycles)||0),lastCycle=Math.max(-1,...resolutionEvents.filter(event=>String(event.actorPlayerId??event.actor_player_id)===player.id&&String(event.abilityId??event.ability_id)===ability.id&&String(event.eventType??event.event_type).toUpperCase()==='ABILITY_CONSUMED').map(event=>Number(event.cycle??event.resolutionCycle??-1))),cooldownUntil=cooldownCycles&&lastCycle>=0?lastCycle+cooldownCycles:null,availabilityState=availability({ability,passive,usesRemaining:grant.usesRemaining,grant,game,cooldownUntil});result.push({key:'GRANT:'+grant.id,abilityId:ability.id,baseAbilityId:ability.id,effectiveAbilityId:amplified?.id||ability.id,name:ability.name,effectiveName:amplified?.name||ability.name,definition:ability.definition,category:ability.category||'Custom',phase:ability.phase||'Any',mechanics:ability.mechanics||[],targeting:abilityTargeting(amplified||ability),sourceType:grant.sourceType,sourceLabel:grant.sourceType.replaceAll('_',' '),sourceReference:grant.sourceReference,grantId:grant.id,grantVersion:grant.version,passive,usesGranted:grant.usesGranted,usesRemaining:grant.usesRemaining,additionalUses:0,cooldownUntil,expiresAt:grant.expiresAt,expiresCycle:grant.expiresCycle,expiresPhase:grant.expiresPhase,reason:grant.reason,modifiers:[],...availabilityState});}
...
export function validateActionTargets({actor,players=[],factions=[],ability,targetIds=[],targetAbilityId='',targetFactionId='',customTarget=''}={}){
  ...
  if(targeting.type==='NO_TARGET'&&ids.length)errors.push('TARGET_NOT_ALLOWED');
  ...
  if(targeting.factionMemberOnly&&targets.some(target=>(target.currentFactionId||target.factionId||'')!==actorFaction))errors.push('FACTION_MEMBER_TARGET_REQUIRED');
  if(targeting.nonFactionMemberOnly&&targets.some(target=>(target.currentFactionId||target.factionId||'')===actorFaction))errors.push('NON_FACTION_TARGET_REQUIRED');
