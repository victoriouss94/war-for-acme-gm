import {TARGET_TYPES as MECHANIC_TARGET_TYPES,normalizeAbilityUnderstanding,normalizeTargeting} from './mechanics.js?v=12.2.55';
import {classifyAbility} from './global-abilities.js?v=12.2.55';
...
export function abilityIdsForRole(role,abilities){
  if(!role||String(role.roleType).toUpperCase()==='BASIC')return [];
  const modeModel=normalizeRoleModes(role,abilities),ids=[role.activeAbilityId,role.passiveAbilityId,...modeModel.roleWideAbilityIds,...modeModel.modes.flatMap(mode=>[...mode.abilityIds,...mode.passiveAbilityIds])].filter(Boolean),tags=new Set((role.tags||[]).map(key));
  for(const ability of abilities)if(tags.has(key(ability.name))&&!isModeContextAbility(ability,role))ids.push(ability.id);
  return [...new Set(ids)];
}
...
function availability({ability,passive,usesRemaining,grant,game,cooldownUntil,additionalReasons=[]}){
  const reasons=[...additionalReasons],phase=clean(game.currentPhase,20),abilityPhase=clean(ability.phase,30);
...
export function effectivePlayerAbilities({player,role,abilities=[],grants=[],statuses=[],resolutionEvents=[],roleModifiers=[],game={},selectedModeId=''}={}){
  if(!player)return {abilities:[],warnings:[],modeContext:roleModeContext({})};const byId=new Map(abilities.map(ability=>[String(ability.id),ability])),result=[],modeContext=roleModeContext({player,role,abilities,grants,statuses,selectedModeId,game});
  for(const abilityId of abilityIdsForRole(role,abilities)){
    const counterRaw=roleAbilityCounter(grants,player.id,role?.id,abilityId),counter=counterRaw?normalizeAbilityGrant(counterRaw):null;const ability=byId.get(String(abilityId));if(!ability||isModeContextAbility(ability,role))continue;const modeAccess=abilityModeAccess({abilityId,context:modeContext}),modeSpecific=modeAccess.modes.length>0,relevantMode=modeAccess.modes.some(mode=>mode.id===modeContext.selectedModeId||modeContext.temporaryModeIds.includes(mode.id));if(modeContext.modes.length&&modeSpecific&&!relevantMode)continue;const passive=String(role?.passiveAbilityId)===String(abilityId)||clean(ability.phase,30)==='Passive'||modeAccess.modes.some(mode=>mode.passiveAbilityIds.includes(String(abilityId))),usageMode=modeAccess.currentMode||modeAccess.temporaryModes[0]||modeAccess.selectedMode,configuredUses=usageMode?.abilityUses?.[ability.id]??usageMode?.abilityUses?.[ability.name]?.baseUses??usageMode?.abilityUses?.[ability.name],baseUses=configuredUses==null?(role?.abilityUses==null?null:Math.max(0,Number(role.abilityUses)||0)):Math.max(0,Number(configuredUses)||0),additional=statusUses(statuses,player.id,ability.id),consumed=consumptionCount(resolutionEvents,player.id,ability.id),usesRemaining=counter?counter.usesRemaining:baseUses==null?null:Math.max(0,baseUses+additional-consumed),amplified=amplifiedAbility(statuses,player.id,ability.id,abilities),cooldownCycles=Math.max(0,Number(ability.cooldownCycles)||0),lastCycle=Math.max(-1,...resolutionEvents.filter(event=>String(event.actorPlayerId??event.actor_player_id)===player.id&&String(event.abilityId??event.ability_id)===ability.id&&String(event.eventType??event.event_type).toUpperCase()==='ABILITY_CONSUMED').map(event=>Number(event.cycle??event.resolutionCycle??-1))),cooldownUntil=cooldownCycles&&lastCycle>=0?lastCycle+cooldownCycles:null,availabilityState=availability({ability,passive,usesRemaining,game,cooldownUntil,additionalReasons:modeSpecific&&!modeAccess.available&&!modeAccess.requiresSwitch?['INACTIVE_MODE']:[]});
    const understanding=normalizeAbilityUnderstanding(amplified||ability),classification=classifyAbility(amplified||ability);result.push({key:'ROLE:'+ability.id,abilityId:ability.id,baseAbilityId:ability.id,effectiveAbilityId:amplified?.id||ability.id,name:ability.name,effectiveName:amplified?.name||ability.name,definition:ability.definition,category:ability.category||'Custom',phase:ability.phase||'Any',mechanics:ability.mechanics||[],understanding,...classification,targeting:abilityTargeting(amplified||ability),sourceType:'ROLE',sourceLabel:modeAccess.roleWide?'Role-wide':modeAccess.modeName||'Role',sourceReference:role?.id||'',modeId:modeAccess.modeId,modeName:modeAccess.modeName,modeIds:modeAccess.modes.map(mode=>mode.id),roleWide:modeAccess.roleWide,requiresModeSwitch:modeAccess.requiresSwitch,grantId:counter?.id||null,grantVersion:counter?.version||null,passive,usesGranted:counter?counter.usesGranted:baseUses,usesRemaining,additionalUses:additional,cooldownUntil,modifiers:roleModifiers.filter(modifier=>String(modifier.role_id??modifier.roleId)===String(role?.id)&&String(modifier.ability_id??modifier.abilityId)===ability.id),...availabilityState});
  }
  for(const rawGrant of grants){if(roleAbilityCounter(grants,player.id,role?.id,rawGrant.abilityId??rawGrant.ability_id)?.id===rawGrant.id)continue;const grant=normalizeAbilityGrant(rawGrant);if(grant.playerId!==player.id||!grantIsCurrent(grant,game))continue;const ability=byId.get(grant.abilityId);if(!ability||isModeContextAbility(ability,role))continue;const configuredModeIds=[...(grant.specialConditions.modeIds||grant.specialConditions.mode_ids||[]),...(grant.metadata.modeIds||grant.metadata.mode_ids||[])].map(String),mode=modeContext.modes.find(item=>configuredModeIds.includes(item.id)),passive=clean(ability.phase,30)==='Passive',amplified=amplifiedAbility(statuses,player.id,ability.id,abilities),cooldownCycles=Math.max(0,Number(ability.cooldownCycles)||0),lastCycle=Math.max(-1,...resolutionEvents.filter(event=>String(event.actorPlayerId??event.actor_player_id)===player.id&&String(event.abilityId??event.ability_id)===ability.id&&String(event.eventType??event.event_type).toUpperCase()==='ABILITY_CONSUMED').map(event=>Number(event.cycle??event.resolutionCycle??-1))),cooldownUntil=cooldownCycles&&lastCycle>=0?lastCycle+cooldownCycles:null,availabilityState=availability({ability,passive,usesRemaining:grant.usesRemaining,grant,game,cooldownUntil,additionalReasons:mode&&!modeContext.accessibleModeIds.includes(mode.id)?['INACTIVE_MODE']:[]}),understanding=normalizeAbilityUnderstanding(amplified||ability),classification=classifyAbility(amplified||ability);result.push({key:'GRANT:'+grant.id,abilityId:ability.id,baseAbilityId:ability.id,effectiveAbilityId:amplified?.id||ability.id,name:ability.name,effectiveName:amplified?.name||ability.name,definition:ability.definition,category:ability.category||'Custom',phase:ability.phase||'Any',mechanics:ability.mechanics||[],understanding,...classification,targeting:abilityTargeting(amplified||ability),sourceType:grant.sourceType,sourceLabel:grant.sourceType.replaceAll('_',' '),sourceReference:grant.sourceReference,modeId:mode?.id||'',modeName:mode?.name||'',modeIds:configuredModeIds,roleWide:!configuredModeIds.length,grantId:grant.id,grantVersion:grant.version,passive,usesGranted:grant.usesGranted,usesRemaining:grant.usesRemaining,additionalUses:0,cooldownUntil,durationType:grant.durationType,grantedCycle:grant.grantedCycle,grantedPhase:grant.grantedPhase,expiresAt:grant.expiresAt,expiresCycle:grant.expiresCycle,expiresPhase:grant.expiresPhase,reason:grant.reason,modifiers:[],...availabilityState});}
  if(abilityDisablingStatuses(statuses,player.id,game).length)for(const item of result)if(!item.passive){item.available=false;item.reasons=[...new Set([...item.reasons,'ABILITIES_DISABLED'])];}
  if(player.alive===false)for(const item of result){item.available=false;item.reasons=['PLAYER_DEAD',...item.reasons.filter(reason=>reason!=='PLAYER_DEAD')];}
  return {abilities:result.sort((a,b)=>a.passive-b.passive||a.category.localeCompare(b.category)||a.name.localeCompare(b.name)),warnings:warningLabels(statuses,player.id),modeContext};
}
...
export function effectiveFactionAbilities({faction,players=[],abilities=[],statuses=[],game={}}={}){
  if(!faction)return [];
  const eligiblePerformers=players.filter(player=>player.alive!==false&&String(player.currentFactionId||player.factionId||'')===String(faction.id)&&!abilityDisablingStatuses(statuses,player.id,game).length&&!statuses.some(status=>String(status.playerId??status.player_id)===String(player.id)&&String(status.state).toUpperCase()==='ACTIVE'&&['DEN_BLOCKED','FACTION_ACTION_BLOCKED'].includes(String(status.statusType??status.status_type).toUpperCase())));
