import {TARGET_TYPES as MECHANIC_TARGET_TYPES,normalizeAbilityUnderstanding,normalizeTargeting} from './mechanics.js?v=11.5.0';
...
export function effectivePlayerAbilities({player,role,abilities=[],grants=[],statuses=[],resolutionEvents=[],roleModifiers=[],game={}}={}){
  ...
  for(const abilityId of abilityIdsForRole(role,abilities)){
    ...
    const understanding=normalizeAbilityUnderstanding(amplified||ability);result.push({key:'ROLE:'+ability.id,abilityId:ability.id,baseAbilityId:ability.id,effectiveAbilityId:amplified?.id||ability.id,name:ability.name,effectiveName:amplified?.name||ability.name,definition:ability.definition,category:ability.category||'Custom',phase:ability.phase||'Any',mechanics:ability.mechanics||[],understanding,targeting:abilityTargeting(amplified||ability),sourceType:'ROLE',sourceLabel:'Role',sourceReference:role?.id||'',grantId:null,grantVersion:null,passive,usesGranted:baseUses,usesRemaining,additionalUses:additional,cooldownUntil,modifiers:roleModifiers.filter(modifier=>String(modifier.role_id??modifier.roleId)===String(role?.id)&&String(modifier.ability_id??modifier.abilityId)===ability.id),...availabilityState});
  }
  for(const rawGrant of grants){const grant=normalizeAbilityGrant(rawGrant);if(grant.playerId!==player.id||!grantIsCurrent(grant,game))continue;const ability=byId.get(grant.abilityId);if(!ability)continue;const passive=clean(ability.phase,30)==='Passive',amplified=amplifiedAbility(statuses,player.id,ability.id,abilities),cooldownCycles=Math.max(0,Number(ability.cooldownCycles)||0),lastCycle=Math.max(-1,...resolutionEvents.filter(event=>String(event.actorPlayerId??event.actor_player_id)===player.id&&String(event.abilityId??event.ability_id)===ability.id&&String(event.eventType??event.event_type).toUpperCase()==='ABILITY_CONSUMED').map(event=>Number(event.cycle??event.resolutionCycle??-1))),cooldownUntil=cooldownCycles&&lastCycle>=0?lastCycle+cooldownCycles:null,availabilityState=availability({ability,passive,usesRemaining:grant.usesRemaining,grant,game,cooldownUntil}),understanding=normalizeAbilityUnderstanding(amplified||ability);result.push({key:'GRANT:'+grant.id,abilityId:ability.id,baseAbilityId:ability.id,effectiveAbilityId:amplified?.id||ability.id,name:ability.name,effectiveName:amplified?.name||ability.name,definition:ability.definition,category:ability.category||'Custom',phase:ability.phase||'Any',mechanics:ability.mechanics||[],understanding,targeting:abilityTargeting(amplified||ability),sourceType:grant.sourceType,sourceLabel:grant.sourceType.replaceAll('_',' '),sourceReference:grant.sourceReference,grantId:grant.id,grantVersion:grant.version,passive,usesGranted:grant.usesGranted,usesRemaining:grant.usesRemaining,additionalUses:0,cooldownUntil,durationType:grant.durationType,grantedCycle:grant.grantedCycle,grantedPhase:grant.grantedPhase,expiresAt:grant.expiresAt,expiresCycle:grant.expiresCycle,expiresPhase:grant.expiresPhase,reason:grant.reason,modifiers:[],...availabilityState});}
...
export function effectiveFactionAbilities({faction,players=[],abilities=[],statuses=[]}={}){
  ...
  return abilities.map(ability=>({ability,understanding:normalizeAbilityUnderstanding(ability)})).filter(item=>item.understanding.factionAction&&(!item.understanding.sourceFactionIds.length||item.understanding.sourceFactionIds.includes(String(faction.id)))).map(({ability,understanding})=>({key:'FACTION:'+ability.id,abilityId:ability.id,baseAbilityId:ability.id,effectiveAbilityId:ability.id,name:ability.name,effectiveName:ability.name,definition:ability.definition,category:ability.category||'Custom',phase:ability.phase||'Any',mechanics:ability.mechanics||[],understanding,targeting:abilityTargeting(ability),sourceType:'FACTION',sourceLabel:'Faction',sourceReference:faction.id,grantId:null,grantVersion:null,passive:false,usesGranted:null,usesRemaining:null,available:!understanding.performerRequired||eligiblePerformers.length>0,reasons:understanding.performerRequired&&!eligiblePerformers.length?['NO_ELIGIBLE_FACTION_PERFORMER']:[],eligiblePerformers}));
}
