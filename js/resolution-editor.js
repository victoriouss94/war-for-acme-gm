...
// Database audit metadata accepts objects or legacy reason strings, never booleans/JSON null.
const overrideMetadata=value=>typeof value==='string'?value:value&&typeof value==='object'&&!Array.isArray(value)?{...value}:{};
...
export function playerOutcomeDisplayRows(outcomes=[],lookup){
  return array(outcomes).filter(outcome=>{
    const playerId=text(outcome?.player_id||outcome?.playerId,200),player=lookup?.players?.find(item=>entityId(item)===playerId)||{},roleChanged=text(outcome?.role_id)!==text(player.roleId||player.role_id),factionChanged=text(outcome?.faction_id)!==text(player.currentFactionId||player.current_faction_id||player.factionId||player.faction_id);
    return text(outcome?.life_state).toUpperCase()!=='UNCHANGED'||roleChanged||factionChanged||array(outcome?.changes).length||text(outcome?.summary);
  }).map(outcome=>{
    ...
    const changes=array(outcome?.changes).map(change=>`${text(change.type||'Change')}: ${text(change.before)||'None'} → ${text(change.after)||'None'}`),state=text(outcome?.life_state).toUpperCase();
    return `${resolutionEntityName(lookup,playerId,'player')} — ${resolutionEntityName(lookup,finalFactionId,'faction')}${state&&state!=='UNCHANGED'?' — '+state:''}${changes.length?' — '+changes.join('; '):outcome.summary?' — '+text(outcome.summary):''}`;
  });
}
...
export function normalizeActionResult(input={},action={},index=0,events=[]){
  ...
  return {
    action_id:actionId,order:integer(input.order??index+1,1,10000),actor_player_id:text(input.actor_player_id||actionActor(action),100),actor_name:text(input.actor_name||action.actorName,200),actor_role_name:text(input.actor_role_name||action.roleName,200),actor_mode_name:text(input.actor_mode_name||action.modeName,200),ability_id:text(input.ability_id||input.selected_ability_id||actionAbility(action),120),ability_name:text(input.ability_name||input.selected_ability_name||action.name,200),selected_ability_id:text(input.selected_ability_id||input.ability_id||actionAbility(action),120),selected_ability_name:text(input.selected_ability_name||input.ability_name||action.name,200),standardized_ability_type:text(input.standardized_ability_type||normalizedAction.standardizedAbilityType,200),resolution_category:resolutionCategory,resolution_priority:resolutionPriority,resolution_timing:resolutionTiming,active_passive:activePassive,ability_source:sourceType(input.ability_source||action.abilitySource||action.sourceType),source_type:sourceType(input.source_type||action.abilitySource||action.sourceType),source_faction_id:text(input.source_faction_id||action.sourceFactionId,100),role_id:text(input.role_id||action.roleId||action.sourceRoleId,100),role_version:integer(input.role_version||action.roleVersion||1,1,100000),source_game_rule:text(input.source_game_rule||normalizedAction.sourceGameRule,500),global_rule_used:text(input.global_rule_used||normalizedAction.globalRuleUsed,500),gm_override:overrideMetadata(input.gm_override??normalizedAction.gmOverride),original_target_ids:strings(input.original_target_ids||normalizedAction.originalTargetIds||actionTargets(action)),original_target_names:strings(input.original_target_names),final_target_ids:strings(input.final_target_ids||input.affected_player_ids||primary?.affected_player_ids||redirect?.affected_player_ids||normalizedAction.effectiveTargetIds||actionTargets(action)),effective_target_names:strings(input.effective_target_names),affected_player_ids:strings(input.affected_player_ids||primary?.affected_player_ids||normalizedAction.effectiveTargetIds||actionTargets(action)),transformation_history:array(input.transformation_history||normalizedAction.transformationHistory).slice(0,100).map(item=>({...item})),generated:Boolean(input.generated??normalizedAction.generated),parent_action_id:text(input.parent_action_id||normalizedAction.parentActionId,160),submitted_attempt:input.submitted_attempt??normalizedAction.submittedAttempt,generated_child_effects:array(input.generated_child_effects).slice(0,100).map(item=>({...item})),generated_effects:array(input.generated_effects).slice(0,100).map(item=>({...item})),passive_triggers:array(input.passive_triggers).slice(0,100).map(item=>({...item})),triggered_passives:array(input.triggered_passives).slice(0,100).map(item=>({...item})),result,ruling:text(input.ruling||input.reason||primary?.summary,4000),reason:text(input.reason||primary?.summary,4000),attempted:input.attempted!==false,redirected:Boolean(input.redirected||redirect),reflected:Boolean(input.reflected||reflection),protected:Boolean(input.protected),immune:Boolean(input.immune),faction_action:Boolean(input.faction_action||String(action.sourceType||'').toUpperCase()==='FACTION'),use_disposition:upper(input.use_disposition||(consumed?'CONSUMED':refunded?'REFUNDED':action.playerAbilityGrantId?'NOT_CONSUMED':'NOT_APPLICABLE'),'NOT_APPLICABLE',USE_RESULTS),player_ability_grant_id:text(input.player_ability_grant_id||action.playerAbilityGrantId,100),cooldown:text(input.cooldown,200)
  };
}
...
export function normalizeStatusEffect(input={}){
  const statusType=text(input.status_type||input.type,64).toUpperCase().replace(/[^A-Z0-9_]/g,'_'),expiryPhase=text(input.expires_at_phase,40);
  // Hanging is a GM-resolved event, not a Day/Night database phase.
  const untilHanging=['DRUNK','SOBER'].includes(statusType)&&expiryPhase.toUpperCase()==='HANGING';
  // Empty numeric controls must stay unspecified through repeated editor passes.
  const optionalTimer=value=>value==null||String(value).trim()===''?'':integer(value,0,9999);
  return {id:text(input.id||crypto?.randomUUID?.()||Math.random(),160),operation:String(input.operation||'APPLY').toUpperCase()==='REMOVE'?'REMOVE':'APPLY',status_id:text(input.status_id,100),player_id:text(input.player_id,100),status_type:statusType,status_name:text(input.status_name||input.name||input.status_type||input.type,120),status_category:text(input.status_category||input.category||'TEMPORARY',40).toUpperCase(),state:text(input.state||'ACTIVE',40).toUpperCase(),source_player_id:text(input.source_player_id,100),source_role_id:text(input.source_role_id,100),source_ability_id:text(input.source_ability_id,120),description:text(input.description,4000),duration:text(input.duration,200)||(untilHanging?'Until Hanging':''),expires_at_cycle:optionalTimer(input.expires_at_cycle),expires_at_phase:untilHanging?'':expiryPhase,remaining_duration:optionalTimer(input.remaining_duration),visibility:text(input.visibility||'GM_ONLY',40).toUpperCase(),reason:text(input.reason,2000)};
}
...
function normalizeLethalAttempts(attempts){
  return array(attempts).filter(item=>item&&typeof item==='object').map(item=>({action_id:text(item.action_id,160),parent_action_id:text(item.parent_action_id,160),root_action_id:text(item.root_action_id||item.action_id,160),actor_player_id:text(item.actor_player_id,120),target_player_id:text(item.target_player_id,120),ability_id:text(item.ability_id,120),ability_name:text(item.ability_name,200),generated:Boolean(item.generated),kill_tier:Number(item.kill_tier)||0,protection_tier:Number(item.protection_tier)||0}));
}
...
export function validateResolutionDraft(draft,{actions=[],players=[],roles=[],abilities=[],factions=[],allowWarnings=false,forApproval=false}={}){
  ...
  if(forApproval&&!allowWarnings&&(text(draft.resolution_status).toUpperCase()==='GM_REVIEW_REQUIRED'||array(draft.unresolved_questions).length))errors.push('Resolve all GM review questions and select a resolved status before approval.');
  if(forApproval&&allowWarnings&&text(draft.why).length<3)errors.push('Finalize With Override requires a GM reason of at least 3 characters in Why.');
  ...
  if(!allowWarnings&&['RESOLVED','RESOLVED_WITH_AI_ASSISTANCE'].includes(text(draft.resolution_status).toUpperCase())&&array(draft.unresolved_questions).length)errors.push('Resolve or remove the unresolved questions before approval.');
  ...
  for(const result of array(draft.action_results)){
    ...
    const queued=actions.find(action=>String(action.id)===String(result.action_id)),selectedName=text(queued?.abilityNameSnapshot||queued?.abilityName||queued?.name),modeName=text(queued?.modeName);if(selectedName&&text(result.ability_name).toLowerCase()!==selectedName.toLowerCase())errors.push(`${selectedName} must remain the selected ability.`);if(modeName&&text(result.ability_name).toLowerCase()===modeName.toLowerCase()&&selectedName.toLowerCase()!==modeName.toLowerCase())errors.push(`${modeName} is mode context, not the action ability.`);
  ...
  for(const status of array(draft.status_effects)){
    if(!playerIds.has(status.player_id))errors.push('A status effect references an unknown player.');
    if(status.operation==='APPLY'&&(!status.status_type||!status.status_name))errors.push('An applied status needs a type and name.');
    if(status.operation==='REMOVE'&&!status.status_id)errors.push('Removing a status requires its saved status ID.');
    if(status.operation==='APPLY'&&status.expires_at_phase&&!['Day','Night','Any'].includes(status.expires_at_phase))errors.push('A status expiry phase must be Day, Night, Any, or blank. Use Duration for event-based timing.');
  }
  ...
  const successfulDeathTargets=new Set(array(draft.action_results).filter(item=>item.result==='SUCCESS').flatMap(item=>item.affected_player_ids));for(const effect of array(draft.other_effects))if(effect.type==='STATUS_CONSEQUENCE'&&effect.value==='DEAD'&&array(draft.status_effects).some(status=>status.operation==='REMOVE'&&status.status_id===effect.target_id&&status.player_id===effect.player_id))successfulDeathTargets.add(effect.player_id);for(const outcome of array(draft.player_outcomes)){if(!playerIds.has(outcome.player_id))errors.push('A player outcome references an unknown player.');if(outcome.life_state==='DEAD'&&!successfulDeathTargets.has(outcome.player_id)){const playerName=players.find(item=>String(item.id)===String(outcome.player_id))?.name||'Unknown player';warnings.push(`${playerName} is marked dead without an explicitly successful effect affecting them.`)}}
...
export function resolutionDifferences(aiDraft={},finalDraft={}){
  ...
  const aiActions=new Map(array(aiDraft.action_results).map(item=>[item.action_id,item]));for(const item of array(finalDraft.action_results)){const before=aiActions.get(item.action_id)||{};for(const key of ['order','standardized_ability_type','resolution_category','resolution_priority','resolution_timing','active_passive','cooldown','source_game_rule','global_rule_used','result','final_target_ids','affected_player_ids','transformation_history','generated_child_effects','passive_triggers','redirected','reflected','protected','immune','use_disposition','reason'])compare(`actions.${item.action_id}.${key}`,before[key],item[key])}
  for(const key of ['title','summary','resolution_order','resolution_status','unresolved_questions','passive_results','status_effects','player_outcomes','faction_results','grant_effects','other_effects','final_ruling','why','authority_used','confidence'])compare(key,aiDraft[key],finalDraft[key]);return changes.slice(0,1000);
}
...
import {GLOBAL_RESOLUTION_ORDER,classifyAndOrderActions,normalizeResolutionAction} from './global-abilities.js?v=12.2.55';
