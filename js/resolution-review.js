...
export function buildTrackerResolutionReview({draft={},roster=[],snapshotPlayers=[],roles=[],factions=[],submittedActions=[],statuses=[]}={}){
  ...
  return {players,tableRows,factionActions,diagnostics,summary:{beforeAlive:players.filter(player=>player.currentAlive).length,afterAlive:players.filter(player=>player.proposedAlive).length,deaths,survived,statusChanges,intel,conversions,pending,reflected,grantChanges},isComplete:pending.length===0};
}
