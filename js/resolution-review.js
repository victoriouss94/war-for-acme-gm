...
export function trackerActionBadges(action={}){
  ...
  if(upper(action.resolution_category)==='KILLS'&&result==='SUCCESS'&&/\bdies\b/i.test(text(action.ruling||action.reason)))keys.push('DEAD');
  if(!keys.length||!['SUCCESS','PENDING'].includes(result))keys.push(result);
  else if(result==='PENDING')keys.push('PENDING');
  else if(result==='SUCCESS'&&!keys.includes('INTEL')&&!keys.includes('DEAD'))keys.unshift('SUCCESS');
...
export function buildTrackerResolutionReview({draft={},roster=[],snapshotPlayers=[],roles=[],factions=[],submittedActions=[],statuses=[]}={}){
  ...
  // Historical comparisons use the session roster, never post-approval live state.
  const snapshotById=new Map(list(snapshotPlayers).map(item=>[idOf(item),item])),liveById=new Map(list(roster).map(item=>[idOf(item),item])),ordered=list(snapshotPlayers).length?list(snapshotPlayers):list(roster),seen=new Set(),players=[];
  for(const item of ordered){
    const playerId=idOf(item);if(!playerId||seen.has(playerId))continue;seen.add(playerId);
    const player={...(liveById.get(playerId)||{}),...item,...(snapshotById.get(playerId)||{})},outcome=outcomeByPlayer.get(playerId),currentRoleId=playerRoleId(player),currentFactionId=playerFactionId(player),proposedRoleId=text(outcome?.role_id??currentRoleId),proposedFactionId=text(outcome?.faction_id)||currentFactionId,currentAlive=player.alive!==false,lifeState=upper(outcome?.life_state),proposedAlive=lifeState==='DEAD'?false:['ALIVE','REVIVED'].includes(lifeState)?true:currentAlive,currentStatuses=currentStatusesFor(statuses,playerId),proposedStatuses=proposedStatusesFor(draft,playerId,currentStatuses);
    ...
    players.push({id:playerId,name:text(player.name)||'Unknown player',currentAlive,proposedAlive,currentRoleId,proposedRoleId,currentRoleName:text(roleById.get(currentRoleId)?.name)||'No role',proposedRoleName:proposedRoleId?(text(roleById.get(proposedRoleId)?.name)||text(outcome?.role_after_resolution)||'Unknown role'):'No role',currentFactionId,proposedFactionId,currentFactionName:text(factionById.get(currentFactionId)?.name)||'No faction',proposedFactionName:text(factionById.get(proposedFactionId)?.name)||'No faction',factionClass:text(factionById.get(proposedFactionId)?.class||factionById.get(currentFactionId)?.class),modeName:text(outcome?.mode_after_resolution||player.modeName||player.mode_name||playerActions[0]?.actor_mode_name),currentStatuses,proposedStatuses,actions:playerActions,timedConsequences:list(draft.other_effects).filter(effect=>effect.type==='STATUS_CONSEQUENCE'&&text(effect.player_id)===playerId),outcome,changes:proposedChanges({player,outcome,currentRoleId,currentFactionId,proposedRoleId,proposedFactionId,currentAlive,proposedAlive,draft})});
  }
  const playerById=new Map(players.map(player=>[player.id,player])),deaths=players.filter(player=>player.currentAlive&&!player.proposedAlive),conversions=players.filter(player=>player.currentFactionId!==player.proposedFactionId),pending=actions.filter(action=>upper(action.result)==='PENDING'),intel=actions.filter(action=>upper(action.resolution_category)==='INTEL'),reflected=actions.filter(action=>action.reflected),attackTargets=unique(Array.isArray(draft.lethal_attempts)?draft.lethal_attempts.filter(attempt=>!actions.some(action=>actionId(action)===text(attempt.root_action_id||attempt.action_id)&&['BLOCKED','CANCELLED','INELIGIBLE_EFFECT','PENDING'].includes(upper(action.result)))).map(attempt=>text(attempt.target_player_id)):actions.filter(action=>upper(action.resolution_category)==='KILLS'&&!['BLOCKED','CANCELLED','INELIGIBLE_EFFECT','PENDING'].includes(upper(action.result))).flatMap(targetIds)),survived=attackTargets.map(id=>playerById.get(id)).filter(player=>player?.proposedAlive),statusChanges=list(draft.status_effects),grantChanges=list(draft.grant_effects),factionActions=actions.filter(action=>!actionActorId(action)||action.faction_action),tableRows=players.flatMap(player=>player.actions.length?[...player.actions.map(action=>({playerId:player.id,playerName:player.name,actionId:action.id,actionLabel:`${action.submittedAbility}${action.submittedTargets.length?` → ${action.submittedTargets.join(', ')}`:''}`,resultLabel:action.conciseResult||upper(action.result).replaceAll('_',' '),badges:action.badges,hasAction:true})),...player.timedConsequences.map(effect=>({playerId:player.id,playerName:player.name,actionId:'',actionLabel:'Timed status consequence',resultLabel:text(effect.summary),badges:[],hasAction:false}))]: [{playerId:player.id,playerName:player.name,actionId:'',actionLabel:'No action',resultLabel:text(player.outcome?.summary)||'—',badges:[],hasAction:false}]);
  const diagnostics={canonicalResolver:text(draft.observability?.canonical_resolver)||'resolveNightDeterministically',submitted:Number(draft.observability?.submitted_action_count)||actions.filter(action=>action.submitted_attempt!==false).length,engineResolved:Number(draft.observability?.engine_resolved_count)||0,aiFallbackCalls:Number(draft.observability?.ai_fallback_call_count??draft.observability?.ai_adjudication_count)||0,unresolved:Number(draft.observability?.unresolved_interaction_count??draft.observability?.gm_review_count)||0,generatedEffects:Number(draft.observability?.generated_effect_count)||0};
  const reviewQuestions=unique(list(draft.unresolved_questions).map(text)),requiresReview=reviewQuestions.length>0||['GM_REVIEW_REQUIRED','RESOLUTION_ERROR'].includes(upper(draft.resolution_status));
  return {players,tableRows,factionActions,diagnostics,reviewQuestions,requiresReview,summary:{beforeAlive:players.filter(player=>player.currentAlive).length,afterAlive:players.filter(player=>player.proposedAlive).length,deaths,survived,statusChanges,intel,conversions,pending,reflected,grantChanges},isComplete:pending.length===0&&!requiresReview};
}
