...
const boundedRecords=(value,limit=100)=>records(value,limit).map(item=>{try{const serialized=JSON.stringify(item);return serialized.length<=30000?JSON.parse(serialized):{}}catch{return {}}}).filter(item=>Object.keys(item).length);
...
export function normalizeResolution(input){
  ...
  return {
    schema_version:Number(input.schema_version)===2?2:undefined,title:text(input.title,200),summary:text(input.summary,4000),resolution_order:strings(input.resolution_order,20),action_results:boundedRecords(input.action_results,200),passive_results:boundedRecords(input.passive_results,100),status_effects:boundedRecords(input.status_effects,100),player_outcomes:boundedRecords(input.player_outcomes,200),faction_results:boundedRecords(input.faction_results,100),grant_effects:boundedRecords(input.grant_effects,100),other_effects:boundedRecords(input.other_effects,100),final_ruling:text(input.final_ruling,12000),why:text(input.why,12000),authority_used:strings(input.authority_used,100),confidence:['HIGH','MEDIUM','LOW'].includes(String(input.confidence).toUpperCase())?String(input.confidence).toUpperCase():'MEDIUM',consistency_warnings:strings(input.consistency_warnings,100),
