import {access,readFile} from 'node:fs/promises';
const files=['index.html','css/main.css','vendor/mammoth.browser.min.js','js/supabase-config.js','js/cloud.js','js/global-abilities.js','js/mechanics.js','js/player-setup.js','js/role-modes.js','js/player-abilities.js','js/phase-controller.js','js/document-import.js','js/statuses.js','js/resolution.js','js/resolution-editor.js','js/resolution-review.js','js/copilot.js','js/knowledge.js','js/app.js','scripts/build-transformers-payload.mjs','supabase/migrations/20260812011716_live_player_status_awareness.sql','supabase/migrations/20260812011926_harden_player_status_mutations.sql','supabase/migrations/20260812020135_consolidated_ai_gm_resolution_learning.sql','supabase/migrations/20260812024815_global_master_gm_ai.sql','supabase/migrations/20260812034700_fix_global_learning_runtime.sql','supabase/migrations/20260812050000_global_learning_default.sql','supabase/migrations/20260814090000_master_gm_agent_foundation.sql','supabase/migrations/20260814093000_master_gm_agent_fk_indexes.sql','supabase/migrations/20260814160000_document_driven_global_rules.sql','supabase/migrations/20260823090000_basic_roles_player_assignment.sql','supabase/migrations/20260823093000_role_assignment_fk_indexes.sql','supabase/migrations/20260824110000_dynamic_player_abilities_action_queue.sql','supabase/migrations/20260824120000_fix_action_queue_ambiguous_references.sql','supabase/migrations/20260824130000_integrated_phase_action_queue.sql','supabase/migrations/20260824193118_universal_role_understanding.sql','supabase/migrations/20260824214102_master_gm_ruling_usage_tracking.sql','supabase/migrations/20260828001801_global_master_ability_resolution_order.sql','supabase/migrations/20260828030000_fix_universal_action_queue_ambiguity.sql','supabase/migrations/20260828050000_multi_mode_action_context.sql','supabase/migrations/20260828070000_rebuild_transformers_roles_from_source.sql','supabase/migrations/20260828071000_restore_transformers_exact_mode_text.sql','supabase/migrations/20260828072000_complete_transformers_active_mode_actions.sql','supabase/migrations/20260828073000_fix_mode_context_identifier_ambiguity.sql','supabase/migrations/20260828080000_global_role_configurations.sql','supabase/functions/_shared/response-parser.js','supabase/functions/_shared/resolution-validator.js','supabase/functions/_shared/ai-service.ts','supabase/functions/_shared/global-resolution.ts','supabase/functions/_shared/global-resolution.test.ts','supabase/functions/_shared/master-gm.js','supabase/functions/gm-copilot/index.ts','supabase/functions/gm-document-import/index.ts','supabase/functions/gm-knowledge-ingest/index.ts'];
files.push('js/night-engine.js','supabase/migrations/20260829200000_deterministic_night_resolution_engine.sql');
files.push('supabase/functions/_shared/usage-accounting.js');
files.push('supabase/migrations/20260912144842_account_for_pregame_document_import.sql');
files.push('js/player-runtime.js','supabase/migrations/20260830234338_player_runtime_counters_and_capture.sql');
files.push('supabase/migrations/20260905173123_stop_active_postgrest_retry_storm.sql');
files.push('supabase/migrations/20260905180000_complete_postgrest_retry_storm_containment.sql');
files.push('supabase/migrations/20260905181500_preserve_proposal_conflict_expiry.sql');
await Promise.all(files.map(file=>access(file)));
await access('.nojekyll'); // GitHub Pages otherwise drops _shared ES modules.
const html=await readFile('index.html','utf8');
for(const file of ['css/main.css','vendor/mammoth.browser.min.js','js/supabase-config.js','js/cloud.js','js/app.js'])if(!html.includes(file))throw new Error(`index.html does not load ${file}`);
const app=await readFile('js/app.js','utf8');
if(!app.includes("from './document-import.js?v=12.2.55'"))throw new Error('app.js does not load consistent source review warnings');
for(const module of ['knowledge','statuses','resolution','player-setup'])if(!app.includes(`from './${module}.js?v=12.0.1'`))throw new Error(`app.js does not load the versioned js/${module}.js`);
if(!app.includes("from './copilot.js?v=12.2.33'"))throw new Error('app.js does not load deterministic Copilot resolution routing');
const copilot=await readFile('js/copilot.js','utf8');
if(!copilot.includes("from '../supabase/functions/_shared/master-gm.js?v=12.2.33'"))throw new Error('Copilot does not load the shared server intent classifier');
if(!app.includes("from './mechanics.js?v=12.2.55'"))throw new Error('app.js does not load scoped mechanic review identities');
if(!app.includes("from './phase-controller.js?v=12.2.71'"))throw new Error('app.js does not load finalization-aware phase controls');
if(!app.includes("from './player-abilities.js?v=12.2.55'"))throw new Error('app.js does not load role-scoped player ability availability');
if(!app.includes("from './resolution-editor.js?v=12.2.71'"))throw new Error('app.js does not load the versioned resolution editor');
if(!app.includes("from './global-abilities.js?v=12.2.55'"))throw new Error('app.js does not load the versioned global resolution module');
if(!app.includes("from './role-modes.js?v=12.2.38'"))throw new Error('app.js does not load role-scoped mode runtime');
const release=JSON.parse(await readFile('package.json','utf8')).version;
if(!html.includes(`js/app.js?v=${release}`))throw new Error('index.html does not load the current app release');
if(!html.includes('js/cloud.js?v=12.2.66'))throw new Error('index.html does not load validated document registration confirmations');
if(!app.includes(`from './night-engine.js?v=${release}'`))throw new Error('app.js does not load the current deterministic night engine');
if(!app.includes("from './resolution-review.js?v=12.2.40'"))throw new Error('app.js does not load the tracker-style resolution review module');
const classifierModules=new Set(['global-abilities','mechanics','document-import','player-abilities','resolution-editor']);
for(const parent of ['app','night-engine','player-abilities']){
  const source=await readFile(`js/${parent}.js`,'utf8');
  if(!source.includes("from './role-modes.js?v=12.2.38'"))throw new Error(`${parent} imports stale mode-aware Intel helper`);
}
for(const parent of ['app','night-engine',...classifierModules]){
  const source=await readFile(`js/${parent}.js`,'utf8');
  for(const match of source.matchAll(/from\s+['"]\.\/([a-z-]+)\.js\?v=([^'"]+)['"]/g)){
    const expected=match[1]==='resolution-editor'?'12.2.71':'12.2.55';
    if(classifierModules.has(match[1])&&match[2]!==expected)throw new Error(`${parent} imports stale standard-ID classifier consumer ${match[1]}`);
  }
}
console.log('Static build verified:',files.join(', '));
