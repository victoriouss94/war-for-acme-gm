...
if(!app.includes("from './document-import.js?v=12.2.45'"))throw new Error('app.js does not load consistent source review warnings');
...
if(!app.includes("from './mechanics.js?v=12.2.45'"))throw new Error('app.js does not load scoped mechanic review identities');
if(!app.includes("from './phase-controller.js?v=12.2.14'"))throw new Error('app.js does not load timed status review support');
if(!app.includes("from './player-abilities.js?v=12.2.45'"))throw new Error('app.js does not load role-scoped player ability availability');
...
for(const parent of ['app','night-engine',...classifierModules]){
  ...
  for(const match of source.matchAll(/from\s+['"]\.\/([a-z-]+)\.js\?v=([^'"]+)['"]/g)){
    if(classifierModules.has(match[1])&&match[2]!== (['mechanics','document-import','player-abilities'].includes(match[1])?'12.2.45':'12.2.37'))throw new Error(`${parent} imports stale standard-ID classifier consumer ${match[1]}`);
  }
}
