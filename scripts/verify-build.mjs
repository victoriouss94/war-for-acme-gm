...
if(!app.includes("from './player-abilities.js?v=12.2.38'"))throw new Error('app.js does not load role-scoped player ability availability');
...
for(const parent of ['app','night-engine',...classifierModules]){
  ...
  for(const match of source.matchAll(/from\s+['"]\.\/([a-z-]+)\.js\?v=([^'"]+)['"]/g)){
    if(classifierModules.has(match[1])&&match[2]!== (match[1]==='player-abilities'?'12.2.38':'12.2.37'))throw new Error(`${parent} imports stale standard-ID classifier consumer ${match[1]}`);
  }
}
