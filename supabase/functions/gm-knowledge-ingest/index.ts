...
Deno.serve(async(req:Request)=>{
  ...
  const authHeader=req.headers.get('Authorization')||'';if(!authHeader.startsWith('Bearer '))return json({error:'Sign in before indexing an official document.','code':'AUTH_REQUIRED'},401,origin);
  ...
  // Claim once in the database, before downloads or paid work. Never retry an
  // ambiguous claim or expire it automatically: either can duplicate spending.
  const claimId=crypto.randomUUID();
  try{
    const claimed=await createServiceClient().rpc('claim_knowledge_ingestion_internal',{target_version_id:versionId,actor_user_id:user.id,target_claim_id:claimId});
    if(claimed.error||typeof claimed.data!=='boolean')return json({error:'Exclusive document processing could not be confirmed. Refresh the library before retrying.','code':'INGESTION_CLAIM_UNAVAILABLE'},503,origin);
    if(!claimed.data)return json({error:'Indexing has already started for this document version. Refresh the library to check its status. If processing was interrupted, upload a new version rather than resubmitting this one.','code':'INGESTION_ALREADY_STARTED'},409,origin);
  ...
  try{
    ...
    const bytes=new Uint8Array(await downloaded.data.arrayBuffer()),fileData=`data:${version.content_type};base64,${bytesToBase64(bytes)}`;
    let reservation;try{reservation=await service.rpc('reserve_ai_usage_internal',{target_game_id:gameId,actor_user_id:user.id,target_feature:'knowledge_ingest',target_model:model,target_request_id:requestId,target_pricing_snapshot:price});}catch{throw new OpenAIServiceError('AI usage reservation could not be confirmed. No AI request was made.',503,'AI_USAGE_RESERVATION_FAILED')}
    if(reservation.error||reservation.data!==requestId){const code=String(reservation.error?.message||'');throw new OpenAIServiceError(code==='AI_MONTHLY_LIMIT_REACHED'?'This game has reached its monthly AI limit.':code==='AI_RATE_LIMIT_REACHED'?'This game has reached its AI request rate limit.':'AI usage reservation could not be confirmed. No AI request was made.',code==='AI_MONTHLY_LIMIT_REACHED'||code==='AI_RATE_LIMIT_REACHED'?429:503,['AI_MONTHLY_LIMIT_REACHED','AI_RATE_LIMIT_REACHED'].includes(code)?code:'AI_USAGE_RESERVATION_FAILED')}
    reserved=true;
