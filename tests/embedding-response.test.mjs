...
for(const valid of [false,true])test('actual ingestion handler '+(valid?'saves correctly associated passages':'does not activate corrupted embeddings'),async t=>{
  ...
  const client={auth:{getUser:async()=>({data:{user:{id:'synthetic-gm'}}})},from(table){const query={select(){return query},eq(){return query},single:async()=>({data:{id:'00000000-0000-4000-8000-000000000001',status:'PROCESSING',source_file_name:'audit.txt',storage_path:'synthetic/audit.txt',content_type:'text/plain',official_documents:{game_id:'synthetic-game',title:'Audit',document_type:'RULES'}}}),maybeSingle:async()=>({data:{member_role:'owner'}})};return query},storage:{from:()=>({download:async()=>({data:new Blob(['Synthetic source'])})})},rpc:async(name,args)=>{calls.push({name,args});return {data:{},error:null}}};
  ...
  if(valid){assert.equal(response.status,200);assert.equal(body.status,'ACTIVE');const saved=calls.find(c=>c.name==='complete_knowledge_ingestion_internal');assert.equal(saved.args.target_chunks[0].embedding[0],0.1);assert.equal(saved.args.target_chunks[1].embedding[0],0.2);}
  else{assert.equal(response.status,502);assert.equal(body.code,'INVALID_EMBEDDING_RESPONSE');assert.ok(!calls.some(c=>c.name==='complete_knowledge_ingestion_internal'));assert.ok(calls.some(c=>c.name==='fail_knowledge_ingestion_internal'));}
});
