...
export async function cloneFixture(configure=()=>{}){
  const sourceData={roles:[structuredClone(source)],abilities:structuredClone(abilities),factions:[{id:'village',name:'Village'}],settings:{labels:{}},rules:[]};
  configure(sourceData);const original=structuredClone(sourceData);
  let payload,sequence=0;
  const context={...roleModes,remapSetupReferences,gameIndex:{games:[{id:'source-game',name:'Source',startingDay:0}]},loadGameData:()=>sourceData,id:()=>`new-${++sequence}`,now:()=>new Date().toISOString(),normalizeMeta:g=>g,normalizeRule:r=>r,baseGameData:()=>({}),GMCloud:{createGame:async data=>{payload=data;return [{}]}},localStorage:{setItem(){}},gameDataKey:()=>'',saveIndex(){},renderGames(){},alert:message=>{throw new Error(message)}};
