...
async function run(t,payload,{reservationError='',accountingFailure=false,embeddingFailure=false,saveFailure=false}={}){
  ...
  t.mock.method(globalThis,'fetch',async(url,options)=>{
    ...
    assert.equal(url,'https://api.openai.com/v1/embeddings');if(embeddingFailure)throw Error('Synthetic embedding failure');return Response.json({data:[{index:0,embedding:Array(1536).fill(0)}]});
  });
...
for(const [name,payload,options,status] of [
  ...
  const reserve=r.saved.find(x=>x.name==='reserve_ai_usage_internal'),records=r.saved.filter(x=>x.name==='complete_ai_usage_internal');
...
test('ingestion accounting failure keeps saved document and warns without paid retry',async t=>{
  ...
  assert.equal(r.calls.filter(c=>c==='complete_claimed_knowledge_ingestion_internal').length,1);
  assert.equal(r.calls.filter(c=>c==='complete_ai_usage_internal').length,1);
  assert.equal(r.calls.filter(c=>c==='https://api.openai.com/v1/responses').length,1);
});
