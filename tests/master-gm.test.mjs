...
import {deterministicLiveAnswer,inferMasterIntent,isWriteIntent,MASTER_GM_MAX_TOOL_CALLS,MASTER_GM_TOOLS,nextConversationContext,requestedPlayerName,resolveMasterEntities,toolsForMasterIntent} from '../supabase/functions/_shared/master-gm.js';
...
test('Master GM server never exposes unrestricted model database access and persists an auditable trace',async()=>{
  ...
  assert.match(shared,/MASTER_GM_MAX_TOOL_CALLS=12/);assert.match(shared,/approvalRequired:true/);assert.match(cloud,/review_ai_change_proposal/);assert.match(app,/Approve & Apply|Approve &amp; Apply/);assert.match(app,/editMasterGmProposal/);assert.match(app,/renderMasterGmActivity/);
