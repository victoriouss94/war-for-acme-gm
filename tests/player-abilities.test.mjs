...
test('cloud and UI extend the existing queue and expose review-only deterministic Master GM flows',async()=>{
  ...
  for(const token of ['playerAbilityState','grantPlayerAbility','bulkGrantPlayerAbilities','grantRandomPlayerAbility','mutatePlayerAbilityGrant','queuePlayerAction','removeQueuedAction','approve_and_apply_resolution'])assert.match(cloud,new RegExp(token));
