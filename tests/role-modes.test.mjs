...
test('11b. production mode validation uses collision-safe SQL identifiers',()=>{const sql=readFileSync(new URL('../supabase/migrations/20260828073000_fix_mode_context_identifier_ambiguity.sql',import.meta.url),'utf8');assert.match(sql,/action_player_id text/);assert.match(sql,/action_ability_id text/);assert.match(sql,/action_mode_id text/);assert.doesNotMatch(sql,/[;\s]player_id text[;\s]/);assert.doesNotMatch(sql,/[;\s]ability_id text[;\s]/);assert.doesNotMatch(sql,/[;\s]mode_id text[;\s]/);assert.match(sql,/grant_source\.player_id=action_player_id/);assert.match(sql,/grant_source\.ability_id=action_ability_id/);assert.match(sql,/effect\.player_id=action_player_id/)});
...
test('role editor parses legacy lines and formats rich configuration blocks',()=>{const parsed=parseRoleModeAssignments('Robot Mode: Guard, Reflection\nAlt Mode: Den Block',abilities,['guard','reflect','den-block','wide']);assert.deepEqual(parsed.errors,[]);assert.deepEqual(parsed.roleWideAbilityIds,['wide']);const record={id:'r',modes:parsed.modes,roleWideAbilityIds:parsed.roleWideAbilityIds};assert.match(formatRoleModeAssignments(record,abilities),/\[Robot Mode\]\nactive: Guard, Reflection/);assert.equal(normalizeRoleModes(record,abilities).modes.length,2);assert.equal(abilityModeAccess({abilityId:'den-block',context:roleModeContext({player:{id:'p'},role:{...record,modeSelectionPolicy:'CHOOSE_BEFORE_ACTION'},abilities,selectedModeId:parsed.modes[1].id})}).modeName,'Alt Mode')});
